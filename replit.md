...
- **Draft Campaigns (DraftCampaigns.tsx)**: Submenu under Campaigns for managing all campaign drafts and completed builds. Features table view with columns: S.No, Campaign Name, Domain, Date/Time, Builder Type (1 Click or Builder 3.0), Status (Draft/In Progress/Completed). Action icons: Resume (for drafts), Edit, Download CSV, Delete. Includes search by name/domain, status filter, and builder type filter. Stats cards show total, drafts, and completed counts. Uses localStorage via historyService for persistence.
- **Connected Domains**: Custom domain management in Web Templates > Connected Websites tab. Features: Add Domain button with modal, domain list with full CRUD, status indicators (green/red dots for connected/not connected), registration date, expiry date with status badges (active/expiring soon/expired), registrar info. Uses RDAP (free, open protocol) for domain WHOIS lookup and DNS resolution check for connection status. Backend endpoints: `/api/domains` (GET/POST), `/api/domains/:id` (PUT/DELETE), `/api/domains/:id/refresh`, `/api/domains/lookup/:domain`. Database table: user_domains.
...
- **Teams Management**: Team collaboration feature (Teams.tsx) allowing users to invite team members via email. Features plan-based team limits (Free: 1, Basic: 2, Lifetime: 3, Pro: 5), role assignment (Owner, Admin, Member), separate sections for active members and pending invitations, and localStorage-based data persistence. Users can invite, remove, and resend invitations to team members. **Invite emails are sent via Postmark API** through `/api/email/team-invite` endpoint.
- **Long Tail Keywords (LongTailKeywords.tsx)**: Submenu under Keywords for generating long-tail keyword variations. Features optional URL input and seed keywords textarea. Uses hybrid generation: autocomplete patterns (prefixes/suffixes/questions) combined with AI-powered variations via gpt-4o-mini (up to 200 keywords). Results display with select/deselect functionality, save list with name, copy to clipboard, and export CSV. Saved Lists tab for managing stored keyword lists. All API endpoints use JWT authentication for user isolation. Database table: long_tail_keyword_lists. Backend endpoints: `/api/long-tail-keywords/generate`, `/api/long-tail-keywords/lists` (GET/POST/DELETE).
...
- **Cron Scheduler**: Background job scheduler (server/cronScheduler.ts) runs two jobs: 1) Hourly ads transparency scraper for pending requests, 2) Daily 9 AM weekly activity summary emails sent to subscribed users (Basic, Pro, Lifetime plans) via Postmark. Manual trigger endpoint: `/api/cron/trigger-daily-reports`.
